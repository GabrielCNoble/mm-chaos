...
void EnRr_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params != LIKE_LIKE_PARAM_3) 
    {
        ...
        if(this->actor.params == LIKE_LIKE_TYPE_SMOL)
        {
            this->actor.scale.y *= 0.2f;
            this->actor.scale.x *= 0.2f;
            this->actor.scale.z *= 0.2f;
            this->collider1.dim.radius *= 0.2f;
            this->collider1.dim.height *= 0.2f;
            this->collider2.dim.radius *= 0.2f;
            this->collider2.dim.height *= 0.2f;
            this->collider2.dim.yShift *= 0.2f;
        }
    } 
    else 
    {
    ...
    else if(this->actor.params >= LIKE_LIKE_TYPE_CHAOS)
    {
        this->actor.colChkInfo.mass = MASS_HEAVY;

        if(Chaos_GetConfigFlag(CHAOS_CONFIG_ALLOW_ENEMY_INFIGHTING))
        {
            this->collider1.base.acFlags |= AC_TYPE_ENEMY;
            this->collider2.base.acFlags |= AC_TYPE_ENEMY;
        }
    }

    this->actionFunc = func_808FAF94;
    func_808FAD1C(this, play);
    gChaosContext.EnRr_SpitPlayer = EnRr_SpitPlayer;
}
...
s16 EnRr_GetWobbleRate(EnRr *this)
{
    switch(this->actor.params)
    {
        case LIKE_LIKE_TYPE_TURBO:
            return TURBO_LIKE_LIKE_WOBBLE_RATE;
        
        case LIKE_LIKE_TYPE_SMOL:
            return SMOL_LIKE_LIKE_WOBBLE_RATE;
    }
    
    return NORMAL_LIKE_LIKE_WOBBLE_RATE;
}

f32 EnRr_GetMoveSpeed(EnRr *this)
{
    switch(this->actor.params)
    {
        case LIKE_LIKE_TYPE_TURBO:
            return TURBO_LIKE_LIKE_MOVE_SPEED;
        
        case LIKE_LIKE_TYPE_SMOL:
            return SMOL_LIKE_LIKE_MOVE_SPEED;
    }
    
    return NORMAL_LIKE_LIKE_MOVE_SPEED;
}

s16 EnRr_GetWobbleIncrement(EnRr *this)
{
    switch(this->actor.params)
    {
        case LIKE_LIKE_TYPE_TURBO:
            return TURBO_LIKE_LIKE_WOBBLE_INCREMENT;
        
        case LIKE_LIKE_TYPE_SMOL:
            return SMOL_LIKE_LIKE_WOBBLE_INCREMENT;       
    }
    
    return NORMAL_LIKE_LIKE_WOBBLE_INCREMENT;
}
...
void EnRr_Move(EnRr* this, f32 arg1) {
    ...
    Audio_PlaySfx_AtPosWithFreq(&this->actor.projectedPos, NA_SE_EN_LIKE_WALK, pitch_scale);
}

/* EnRr_SetupLeanTowardsPlayer? */
void func_808FA260(EnRr* this) {
    ...
    this->unk_1E1 = 1;
    this->unk_1E6 = 20;
    this->unk_1F6 = NORMAL_LIKE_LIKE_WOBBLE_RATE;
    if(this->unk_1F4 > this->unk_1F6)
    {
        this->unk_1F4 = this->unk_1F6;
    }
    ...
    Audio_PlaySfx_AtPosWithFreq(&this->actor.projectedPos, NA_SE_EN_LIKE_UNARI, pitch_scale);
...
void func_808FA344(EnRr* this) {
    ...
    this->unk_1E1 = 0;
    this->unk_210 = 0.0f;
    this->unk_1F6 = EnRr_GetWobbleRate(this);
...
void EnRr_SpitPlayer(EnRr* this, PlayState* play) {
    ...
    u32 chaos_like_like = this->actor.params == LIKE_LIKE_TYPE_CHAOS || this->actor.params == LIKE_LIKE_TYPE_VORE;
    ...
    if (player->stateFlags2 & PLAYER_STATE2_80) {
        ...
        if ((!this->swallowedShield && (GET_PLAYER_FORM == PLAYER_FORM_HUMAN)) &&
            (GET_CUR_EQUIP_VALUE(EQUIP_TYPE_SHIELD) == EQUIP_VALUE_SHIELD_HERO) && 
            !Chaos_IsCodeActive(CHAOS_CODE_INVINCIBLE)) {
            /* only steal shield if player has it and is not invincible */
            shield_stolen = true;
            this->swallowedShield = Inventory_DeleteEquipment(play, EQUIP_VALUE_SHIELD_HERO);
        } else {
            shield_stolen = false;
        }
        ...
        if (this->actor.params == LIKE_LIKE_PARAM_0 || chaos_like_like) {
            player_damage = 8;
        } else {
            player_damage = 16;
        }
...
s32 func_808FAA94(EnRr* this, PlayState* play) {
    ...
    if (flag || (this->collider2.base.acFlags & AC_HIT)) {
        ...
        if(sp2C->base.ac->id == ACTOR_EN_RR)
        {
            return false;
        }
...
void func_808FAF94(EnRr* this, PlayState* play) {
    ...
    if ((this->unk_1E6 == 0) && !(player->stateFlags2 & PLAYER_STATE2_80) && (Player_GetMask(play) != PLAYER_MASK_STONE) &&
        ...
        EnRr_Move(this, EnRr_GetMoveSpeed(this));
    }
}

/* EnRr_LeanTowardsPlayer? */
void func_808FB088(EnRr* this, PlayState* play) {
    Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 10, 0x1F4, 0);
    this->actor.world.rot.y = this->actor.shape.rot.y;
...
void EnRr_Update(Actor* thisx, PlayState* play) {
    ...
    if (!func_808FAA94(this, play) && this->actor.params != LIKE_LIKE_TYPE_SMOL) {
    ...
    if (this->actor.params == LIKE_LIKE_PARAM_2) {
        ...
        if(this->actor.params == LIKE_LIKE_TYPE_TURBO)
        {
            Math_StepToF(&this->actor.speed, 0.0f, TURBO_LIKE_LIKE_DEACCELERATION);
        }
        else
        {
            Math_StepToF(&this->actor.speed, 0.0f, NORMAL_LIKE_LIKE_DEACCELERATION);
        }
    }
    ...
    if ((this->actionFunc != func_808FB42C) && (this->actionFunc != func_808FB398)) {
        ...
        if ((this->unk_1F0 == 0) && (this->actionFunc == func_808FB088) && 
            (this->unk_1EE == 0) && this->actor.params != LIKE_LIKE_TYPE_SMOL) {
    ...
    if (this->unk_1EE == 0) {
        Math_ScaledStepToS(&this->unk_1F4, this->unk_1F6, EnRr_GetWobbleIncrement(this));
