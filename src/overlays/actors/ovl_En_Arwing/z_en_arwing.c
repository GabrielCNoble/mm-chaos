...
#define FLAGS (ACTOR_FLAG_ATTENTION_ENABLED | ACTOR_FLAG_HOSTILE | ACTOR_FLAG_HOOKSHOT_PULLS_ACTOR | ACTOR_FLAG_IGNORE_QUAKE | ACTOR_FLAG_MINIMAP_ICON_ENABLED)

void EnArwing_Init(Actor* thisx, PlayState* play);
void EnArwing_Destroy(Actor* thisx, PlayState* play);
void EnArwing_Update(Actor* thisx, PlayState* play2);
void EnArwing_Draw(Actor* thisx, PlayState* play);
...
ActorProfile En_Arwing_Profile = {
    /**/ ACTOR_EN_ARWING,
    /**/ ACTORCAT_BOSS,
    /**/ FLAGS,
    /**/ OBJECT_ARWING,
...
// #include "assets/overlays/ovl_En_Arwing/ovl_En_Arwing.c"
...
void EnArwing_CreateDebrisEffect(PlayState* play, Vec3f* position, Vec3f* velocity, Vec3f* acceleration, f32 scale,
                                   f32 floorHeight) {
    // s16 i;
    // EnArwingEffect* effect = (EnArwingEffect*)play->specialEffects;

    // // Look for an available effect to allocate a Debris effect to.
    // for (i = 0; i < ARWING_EFFECT_COUNT; i++, effect++) {
    //     if (effect->type == ARWING_EFFECT_AVAILABLE) {
    //         effect->type = ARWING_EFFECT_DEBRIS;
...
void EnArwing_Init(Actor* thisx, PlayState* play) {
    ...
    random_direction.x = Rand_CenteredFloat(2.0f);
...
void EnArwing_UpdateEffects(PlayState* play) {
    ...
    //         if (effect->type == ARWING_EFFECT_DEBRIS) {
    ...
    //             if (effect->timer == 1) {
    //                 effect->type = ARWING_EFFECT_AVAILABLE;
    //             }
    ...
    //             }
    //         } else if (effect->type == ARWING_EFFECT_FIRE) {
    ...
    //                 effect->type = ARWING_EFFECT_AVAILABLE;
    //             }
    //         } else if (effect->type == ARWING_EFFECT_SMOKE) {
    ...
    //                 }
    //             }
    //         } else if (effect->type == ARWING_EFFECT_FLASH) {
...
void EnArwing_DrawEffects(PlayState* play) {
    ...
    // u8 materialFlag = 0;
    // EnArwingEffect* effect = (EnArwingEffect*)play->specialEffects;
    // EnArwingEffect* firstEffect = effect;
    ...
    // materialFlag = 0;
    // for (i = 0; i < ARWING_EFFECT_COUNT; i++, effect++) {
    //     if (effect->type == ARWING_EFFECT_FLASH) {
    ...
    // materialFlag = 0;
    // for (i = 0; i < ARWING_EFFECT_COUNT; i++, effect++) {
    //     if (effect->type == ARWING_EFFECT_SMOKE) {
    ...
    // materialFlag = 0;
    // for (i = 0; i < ARWING_EFFECT_COUNT; i++, effect++) {
    //     if (effect->type == ARWING_EFFECT_FIRE) {
