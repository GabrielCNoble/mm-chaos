...
#include "z_en_dark_link.h"
#include "assets/objects/object_dark_link/object_dark_link.h"
#include "z64player.h"
#include "PR/gbi.h"
#include "chaos_fuckery.h"

// #define FLAGS (ACTOR_FLAG_1 | ACTOR_FLAG_2 | ACTOR_FLAG_4 | ACTOR_FLAG_5)
#define FLAGS 0
...
typedef enum {
    ...
    /* 2 */ ENTORCH2_DEATH,
    /* 3 */ ENTORCH2_DAMAGE
} EnDarkLinkActionStates;

void EnDarkLink_Init(Actor* thisx, PlayState* play2);
void EnDarkLink_Destroy(Actor* thisx, PlayState* play);
void EnDarkLink_Update(Actor* thisx, PlayState* play2);
void EnDarkLink_Draw(Actor* thisx, PlayState* play2);

ActorProfile En_Dark_Link_Profile = {
    /**/ ACTOR_EN_DARK_LINK,
    /**/ ACTORCAT_BOSS,
    /**/ FLAGS,
    /**/ OBJECT_DARK_LINK,
...
void EnDarkLink_Init(Actor* thisx, PlayState* play2) {
    ...
    this->heldItemAction = 0;
    this->ageProperties = &sPlayerAgeProperties[PLAYER_FORM_FIERCE_DEITY];
    ...
    this->meleeWeaponEffectIndex[0] = EFFECT_MAX;
    this->meleeWeaponEffectIndex[1] = EFFECT_MAX;
    this->meleeWeaponEffectIndex[2] = EFFECT_BLURE1;
...
Actor* EnDarkLink_GetAttackItem(PlayState* play, Player* this) {
    ...
    {
        return Actor_GetExplosiveActor(play, &this->actor);
    }
}

s32 EnDarkLink_SwingSword(PlayState* play, Input* input, Player* this) {
...
void EnDarkLink_Backflip(Player* this, Input* input, Actor* thisx) {
...
void EnDarkLink_Update(Actor* thisx, PlayState* play2) {
    ...
    attackItem = EnDarkLink_GetAttackItem(play, this);
    switch (sActionState) 
    {
        case ENTORCH2_WAIT:
                ...
                if (((player->meleeWeaponState != 0) || (player->actor.velocity.y > -3.0f)) &&
                    ...
                    if (((u32)sSwordJumpTimer == 0) ||
                        ...
                        if (sSwordJumpTimer < 16) {
                            EnDarkLink_SwingSword(play, input, this);
                            sSwordJumpState++;
                        } else if (sSwordJumpTimer == 19) {
                            // func_800F4190(&this->actor.projectedPos, NA_SE_VO_LI_AUTO_JUMP);
                            AudioSfx_PlaySfx(NA_SE_VO_LI_AUTO_JUMP, &this->actor.projectedPos, 4,
                    ...
                    if (func_800BE184(play, &this->actor, 120.0f, 0x7FFF, 0x7FFF, this->actor.world.rot.y)) {
                        if ((player->meleeWeaponAnimation == PLAYER_MWA_STAB_1H) &&
                            ...
                            if ((this->meleeWeaponState == 0) && (sCounterState == 0) &&
                                ...
                                SkelAnime_Update(&player->skelAnime);
                                sHoldShieldTimer = 0;
                                input->cur.button = BTN_A;
                            } else {
                                EnDarkLink_Backflip(this, input, &this->actor);
                            }
                        } else {
                        ...
                        if ((90.0f >= this->actor.xzDistToPlayer) && (this->actor.xzDistToPlayer > 70.0f) &&
                            ...
                            if (((this->actor.xzDistToPlayer <= 70.0f) ||
                                ...
                                if (!EnDarkLink_SwingSword(play, input, this) && (this->meleeWeaponState == 0) &&
                                    (sCounterState == 0)) {
                                    EnDarkLink_Backflip(this, input, &this->actor);
                                }
        ...
        !(this->meleeWeaponQuads[0].base.atFlags & AT_HIT) && !(this->meleeWeaponQuads[1].base.atFlags & AT_HIT)) 
    {
        ...
        if (!Actor_ApplyDamage(&this->actor)) {
            ...
            this->unk_B75 = 6.0f;
            this->unk_B7C = 6.0f;
            this->unk_B78 = this->actor.colChkInfo.damage;
    ...
    {
        Chaos_ConsolePrintf("Action func is null");
    }
    ...
    if (this->speedXZ == -18.0f) {
        ...
        if (staggerThreshold < sStaggerCount) {
            ...
            AudioSfx_PlaySfx(NA_SE_PL_DAMAGE, &this->actor.projectedPos, 4,
                                &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            sStaggerTimer = 0;
            sStaggerCount = 0;
        }
...
s32 EnDarkLink_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx, Gfx** gfx) {
    Player* this = (Player*)thisx;

    return Player_OverrideLimbDrawGameplayCommon(play, limbIndex, dList, pos, rot, &this->actor);
}

void EnDarkLink_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx, Gfx** gfx) {
    Player* this = (Player*)thisx;
...
void EnDarkLink_Draw(Actor* thisx, PlayState* play2) {
    ...
    if (sAlpha == 255) {
        ...
        POLY_OPA_DISP = SkelAnime_DrawFlex(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                               EnDarkLink_OverrideLimbDraw, EnDarkLink_PostLimbDraw, &this->actor, POLY_OPA_DISP);
    } 
    else 
    {
        ...
        POLY_XLU_DISP = SkelAnime_DrawFlex(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                               EnDarkLink_OverrideLimbDraw, EnDarkLink_PostLimbDraw, &this->actor, POLY_XLU_DISP);
    }
